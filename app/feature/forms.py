...
class FeaturedStoryForm(FlaskForm):
    ...
    left_right = RadioField(
        "Picture on Left or Right",
        choices=[("left", "Picture on Left Side"), ("right", "Picture on Right Side")],
        validators=[DataRequired()],
        default="left",
    )
    title = StringField("Title/Position", validators=[DataRequired()])
    description = TextAreaField(
        "Insert a text here", validators=[DataRequired(), Length(max=365)]
    )
...
class ModifyFeatureForm(FlaskForm):
    ...
    is_visible = RadioField(
        "Visibility",
        choices=[("True", "Visible"), ("False", "Not Visible")],
        validators=[DataRequired()],
        default="True",
    )
    rank = SelectField("Rank", choices=[], default=0, coerce=int)
