...
new_options = old_options + (
    ...
    "login_failed",
    "login_success",
    "featured_story_added",
    "featured_story_edited",
    "featured_story_hidden",
    "email_sent",
)
...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            "left_right",
            sa.Enum("left", "right", name="photo_position"),
            nullable=False,
        ),
    ...
    op.execute(
        "ALTER TABLE events ALTER COLUMN type TYPE _type" " USING type::TEXT::_type"
    )
    old_type.drop(op.get_bind(), checkfirst=False)
    # Create and convert to the "new" type type
    new_type.create(op.get_bind(), checkfirst=False)
    op.execute(
        "ALTER TABLE events ALTER COLUMN type TYPE event_type"
...
def downgrade():
    ...
    op.execute(
        "ALTER TABLE events ALTER COLUMN type TYPE event_type" " USING type::TEXT::type"
    )
    tmp_type.drop(op.get_bind(), checkfirst=False)

    op.create_foreign_key(
        "events_user_guid_fkey", "events", "users", ["user_guid"], ["guid"]
    )
    # ### end Alembic commands ###
